...
export const GeneratePDFBilPayment = async (formData) => {
    ...
    doc.setFont("roboto");
          ...
          doc.text('(VAT INVOICE)', 105, 45, { align: 'center' });
          doc.setTextColor(...colors.black);
...
const textCurrentDate = `Ngày (Date) ${day} tháng (month) ${month} năm (year) ${year}`;
          ...
          autoTable(doc, {
            ...
            body: [
                ['1', 'Điện tiêu thụ từ ngày '+ formData.fromDate +' đến ngày ' + formData.toDate, 'kWh','1', formData.price,formData.totalAmount]
            ],
            styles: {
                font: "roboto",  // Đảm bảo sử dụng font đã tích hợp
                cellPadding: 5,
