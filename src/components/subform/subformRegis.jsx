    ...
    debounce(async (identityCard) => {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          setIdentityCardError(
            res.data.exists ? "CCCD đã tồn tại" : "CCCD thích hợp"
          );
        }
      } catch (error) {
      ...
      createAccount(newAccount).then((res) => {
        if (res.status === 201) {
          ...
          createClient(newClient).then((res) => {
            if (res.status === 201) {
              notifySuccess(
                "Đăng ký thông tin khách hàng thành công! Vui lòng đăng nhập."
              );
              onClose();
              openSubformLogin();
            } else if (res.status === 404) {
              notifyError(
                "Đăng ký thông tin khách hàng thất bại! Kiểm tra lại thông tin"
              );
            }
          });
        } else if (res.status === 404) {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-2">
            ...
            <div className="flex">
              ...
              ></InputCustome>
              <FaEyeSlash
                className={`cursor-pointer absolute end-0 mr-8 mt-3 ${
